...
mata:
real colvector _rd2d_loc_kernel(real colvector u, string scalar kernel)
{
    real colvector a, w
    ...
    string scalar fallbackname)
{
    ...
    real matrix X, bread, ibread, beta, Xw, Xwr, V, meat, score
    real rowvector diag
    real scalar n, k, hr, sd1, sd2, hascluster, g, j, factor
    ...
    if (ktype == "prod") {
        w = _rd2d_loc_kernel(xc1 :/ h1, kernel) :* _rd2d_loc_kernel(xc2 :/ h2, kernel) :/ (h1 * h2)
    }
    else {
        hr = sqrt(h1^2 + h2^2)
        w = _rd2d_loc_kernel(sqrt(xc1:^2 + xc2:^2) :/ hr, kernel) :/ (hr^2)
    }
